...
def upgrade() -> None:
    ...
    CREATE TABLE normalized_observations (
      ...
      provenance jsonb NOT NULL CHECK(coalesce(jsonb_typeof(provenance)='object' AND provenance ?& ARRAY['collector_id','collector_version','adapter_id','adapter_version','normalizer_version','scenario_id','scenario_sha256'] AND provenance->>'collector_id' ~ '^[a-z][a-z0-9._-]{1,63}$' AND provenance->>'adapter_id' ~ '^[a-z][a-z0-9._-]{1,63}$' AND provenance->>'collector_version' ~ '^[0-9]+\.[0-9]+\.[0-9]+$' AND provenance->>'adapter_version' ~ '^[0-9]+\.[0-9]+\.[0-9]+$' AND provenance->>'normalizer_version' ~ '^[0-9]+\.[0-9]+\.[0-9]+$' AND length(provenance->>'scenario_id') BETWEEN 1 AND 128 AND provenance->>'scenario_sha256' ~ '^[a-f0-9]{64}$',false)), freshness_seconds integer NOT NULL CHECK(freshness_seconds>=0),
