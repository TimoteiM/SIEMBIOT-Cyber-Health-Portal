...
Fixture laundering is blocked through immutable mode fields, database checks, composite mode foreign keys, application validation, non-publishable `DEMO/FIXTURE` responses, and a database publication view that excludes fixture rows. Cross-tenant hash enumeration is blocked by forced RLS and tenant-scoped uniqueness. Evidence/finding history is append-only through restricted application grants and database triggers. A separately credentialed worker role can append only within its session tenant; the API role cannot forge generated evidence. Decision events are actor-, scope-, lifecycle-, and tenant-audit-bound. Remaining risks include database-owner compromise, methodology expert validation, production secret/key custody, restricted-egress worker deployment, and independent penetration testing.
