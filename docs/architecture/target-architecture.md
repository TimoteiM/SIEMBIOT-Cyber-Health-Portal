...
Milestone 4 adds canonical evidence identities, allowlisted normalizers, an immutable and duplicate-safe reviewed policy catalog, pure evaluation/scoring, typed score-change attribution, and immutable finding history. PostgreSQL migration `0006_evidence_scoring` retains complete collector/adapter/scenario provenance, evaluation assurance inputs, applicability, pillar/cap outputs, and attribution deltas; it also forces tenant RLS, composite tenant/mode/audit references, fixture publication constraints, and update/delete rejection for evidence lineage. The interactive API role is read-only for generated evidence; only the separately credentialed worker role may append evidence in an explicitly selected tenant. Finding fingerprints are recomputed at runtime and in PostgreSQL. Finding decisions are restricted to owner/security-administrator roles, database-bound to the authenticated actor and finding scope, and serialized per finding. Content hashes are tenant-scoped identifiers, never authorization credentials or global search keys.
