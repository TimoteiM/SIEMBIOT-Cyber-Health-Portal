...
def test_verification_destination_is_fixed_and_structured() -> None:
    ...
    assert destination.path == "/.well-known/tyche-verification.txt"
...
@pytest.mark.parametrize(
    ("location", "reason"),
    [
        ("http://example.com/.well-known/tyche-verification.txt", "tls_downgrade"),
        ("https://user:pass@example.com/.well-known/tyche-verification.txt", "credentials"),
        ("https://example.com/.well-known/tyche-verification.txt#fragment", "fragment"),
        ("ftp://example.com/.well-known/tyche-verification.txt", "unsupported_scheme"),
        ("https://example.com:8443/.well-known/tyche-verification.txt", "forbidden_port"),
        ("https://example.com/other", "forbidden_path"),
        ("https://example.com/.well-known/tyche-verification.txt?token=x", "query"),
        ("https://EXAMPLE.com/.well-known/tyche-verification.txt", "noncanonical_host"),
        ("https://example.com./.well-known/tyche-verification.txt", "noncanonical_host"),
        ("https://127.0.0.1/.well-known/tyche-verification.txt", "ip_literal"),
    ],
)
...
def test_redirect_is_rebuilt_and_requires_exact_authorized_host() -> None:
    ...
    delegated = authorize_redirect(
        current,
        "https://verify.example.net/.well-known/tyche-verification.txt",
    ...
    with pytest.raises(DestinationPolicyError, match="redirect_not_authorized"):
        authorize_redirect(
            current,
            "https://child.example.com/.well-known/tyche-verification.txt",
            authorized_hosts={"example.com"},
        )
